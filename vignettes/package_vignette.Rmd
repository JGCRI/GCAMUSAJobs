...
```{r}
head(elec_gen_activity %>% filter(region == "TX", Year == 2050), 10)
head(elec_gen_activity %>% filter(region == "CA", Year == 2050), 10)
```
...
```{r}
JOB_activity3 <- GCAM_JOB(GW_output, "Net")
```
...
```{r, warning=FALSE, fig.width=18, fig.height=6, fig.alt=" "}
PLOT_EF()
```
...
National level
```{r, warning=FALSE, fig.width=12, fig.height=6, fig.alt=" "}
PLOT_GW(GW_activity)
```


State level (e.g., Colorado)
```{r, warning=FALSE, fig.width=12, fig.height=6, fig.alt=" "}
...
```{r, warning=FALSE, fig.width=16, fig.height=12, fig.alt=" "}
